...
export default class Datatable extends LightningElement {
    ...
    processSelectedRows(){
        if(this.selectedRowsCollection.length <= 0){
            return;
        }
        this.currentSelectedRows = this.selectedRowsCollection.find(item => item.page == this.pageNumber).rows;
    }

    handlerowselection(event) {
        let currentSelections = [];
        for(let i=0; i<event.detail.selectedRows.length; i++){
            currentSelections.push(event.detail.selectedRows[i].Id);
        }
        if(!this.selectedRowsCollection.find(item => item.page == this.pageNumber)){
            this.selectedRowsCollection.push({
                "page": this.pageNumber,
                "rows": currentSelections
            })
        }else{
            this.selectedRowsCollection.find(item => item.page == this.pageNumber).rows = currentSelections;
        }
        
        this.dispatchEvent(new CustomEvent('rowselection', {
            detail: {
                rows: this.selectedRowsCollection
            }
        }));
